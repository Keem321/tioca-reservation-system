...
reservationRouter.get("/", requireAuth, requireRole("manager"), getReservations);

// Get upcoming check-ins (manager only)
reservationRouter.get("/upcoming-checkins", requireAuth, requireRole("manager"), getUpcomingCheckIns);

// Get current check-outs (manager only)
reservationRouter.get("/current-checkouts", requireAuth, requireRole("manager"), getCurrentCheckOuts);
...
reservationRouter.put("/:id", requireAuth, requireRole("manager"), updateReservation);

// Update reservation status (manager only)
reservationRouter.patch("/:id/status", requireAuth, requireRole("manager"), updateReservationStatus);
...
reservationRouter.post("/:id/check-in", requireAuth, requireRole("manager"), checkIn);

// Check out a reservation (manager only)
reservationRouter.post("/:id/check-out", requireAuth, requireRole("manager"), checkOut);

// Delete a reservation (manager only)
reservationRouter.delete("/:id", requireAuth, requireRole("manager"), deleteReservation);
