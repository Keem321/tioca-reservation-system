...
// Single-hotel system: no hotel document required; operate directly on rooms
...
function createRoomsForFloor({ floorKey, floorDigit, distributions }) {
  ...
  for (const dist of distributions) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      rooms.push({
        podId,
        roomNumber: podId, // legacy field mirrors podId for convenience
...
({
  ...
  firstFive: allRooms.slice(0, 5).map(r => ({ podId: r.podId, floor: r.floor, quality: r.quality })),
});
